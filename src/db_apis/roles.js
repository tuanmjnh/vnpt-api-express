...
module.exports.selectRoleRoute = async function(context) {
  const sql = `SELECT ROLE_ID "role_id",ROUTE "route" FROM TTKD_BKN.ROLE_ROUTE WHERE ROLE_ID=:role_id`
  const rs = await db.execute(sql, context)
  return rs.rows
}

module.exports.deleteRoleRoute = async function(context) {
  let sql = `DELETE TTKD_BKN.ROLE_ROUTE WHERE ROLE_ID=:role_id`
...
module.exports.insertRoleRoute = async function(context) {
  ...
  if (context.length) {
    sql = `INSERT INTO TTKD_BKN.ROLE_ROUTE(ROLE_ID,ROUTE) 
  SELECT :role_id,:route FROM DUAL
  WHERE NOT EXISTS(SELECT * FROM TTKD_BKN.ROLE_ROUTE where ROLE_ID=:role_id and ROUTE=:route)`
