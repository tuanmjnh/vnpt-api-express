...
module.exports.get = async function(req, res, next) {
  try {
    ...
    if (!verify) return
    const rs = await dbapi.findNguoiDung({ nguoidung_id: verify.id })
    if (!rs || rs.length < 0) return res.status(401).json({ msg: 'exist', params: 'data' }).end()
    const roles = await dbapiRoles.selectRoleRoute({ role_id: rs[0].roles_id })
    if (rs) return res.status(200).json({ user: rs[0], routes: roles.map(x => x.route) }).end()
    else return res.status(401).json({ msg: 'exist', params: 'data' }).end()
  } catch (e) {
    console.log(e)
...
module.exports.post = async function(req, res, next) {
  try {
    // check req data
    if (!req.body.username) return res.status(401).json({ msg: 'exist' }).end()
    // throw new Error('wrong')
    let rs = await dbapi.getNguoidung({ username: req.body.username })
    // not exist username
    if (!rs || rs.length < 1) return res.status(401).json({ msg: 'exist' }).end()
    // check password
    if (rs[0].giaima_mk !== req.body.password) return res.status(401).json({ msg: 'wrong' }).end()
    // check lock
    ...
    else return res.status(401).json({ msg: 'wrong' }).end()
  } catch (e) {
    console.log(e)
    // next(e)
    return res.status(500).send('invalid')
